...
function switchMode(mode) {
    ...
    // Sélectionne le bon bouton pour ajouter la classe active
...
function lancerGenerationAuto() {
    ...
    // ON NE CHANGE PLUS D'ONGLET ICI
    analyserTout();
}

function resetTout() {
    if(!confirm("Réinitialiser UNIQUEMENT les analyses ? (Infos patient conservées)")) return;

    // Décocher les scénarios
    document.querySelectorAll('.scenario-grid input').forEach(el => el.checked = false);
    
    // Vider les inputs d'analyses seulement
    document.querySelectorAll('.analysis-input').forEach(el => el.value = "");

    // Cacher les lignes et sections du document
