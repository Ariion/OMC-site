...
const database = {
    ...
    "Gaz du Sang (AA)": [
        ...
        { id: "pco2", label: "PCO2", unit: "mmHg", norm: "35 - 45", help: "Respiration." },
    ...
    "Toxicologie (LSPD/BCSO)": [
        { id: "alc", label: "Alcoolémie", unit: "g/L", norm: "0 - 0.10", help: "Taux." },
...
const causesData = {
    ...
    "Toxique": ["Intoxication médicamenteuse massive", "Overdose"]
};
...
// ==========================================
// 4. LOGIQUE GÉNÉRATEUR AUTO & RESET
// ==========================================
...
function resetTout() {
    if(!confirm("Réinitialiser UNIQUEMENT les analyses ?")) return;
...
// LOGIQUE DÉCÈS (Pour ne pas casser l'autre page)
let typeSelectionne = "";
function updateCausesSub(type) {
    ...
    if (causesData[type]) { causesData[type].forEach(c => { select.innerHTML += `<option value="${c}">${c}</option>`; }); }
}
function genererReference() {
    const n = new Date();
    const ref = `${n.getDate()}${n.getMonth()+1}${n.getHours()}${n.getMinutes()}`;
    const el = document.getElementById('d-ref'); if(el) el.innerText = ref;
}
